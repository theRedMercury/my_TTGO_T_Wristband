...
JSONVar::JSONVar(struct cJSON* json, struct cJSON* parent) : _json(json), _parent(parent)
{
}
...
JSONVar::JSONVar(char i) : JSONVar()
{
    *this = i;
}

JSONVar::JSONVar(unsigned char i) : JSONVar()
{
    *this = i;
}

JSONVar::JSONVar(short i) : JSONVar()
{
    *this = i;
}

JSONVar::JSONVar(unsigned short i) : JSONVar()
{
    *this = i;
}
...
JSONVar::JSONVar(unsigned int i) : JSONVar()
{
    *this = i;
}
...
JSONVar::operator float() const
{
    return cJSON_IsNumber(_json) ? static_cast<float>(_json->valuedouble) : NAN;
}
...
{
    return cJSON_Compare(_json, v._json, 1) || (_json == NULL && v._json == NULL);
}
