...
const struct hw_s hw = {.addr = 0x69,
                        .max_fifo = 1024,
                        .num_reg = 128,
                        .temp_sens = 321,
                        .temp_offset = 0,
...
#ifdef AK89xx_SECONDARY
        mpu_set_compass_sample_rate(st.chip_cfg.compass_sample_rate < MAX_COMPASS_SAMPLE_RATE
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            if (debug)
            {
                log_i("Bias_Shift=%7.4f, Bias_Reg=%7.4f, Bias_HWST=%7.4f\r\n", st_shift_cust[i] / 1.f,
                      bias_regular[i] / 1.f, bias_st[i] / 1.f);
            ...
            if (debug)
                log_i("ratio=%7.4f, threshold=%7.4f\r\n", st_shift_ratio[i] / 1.f, test.max_accel_var / 1.f);
...
static int gyro_6500_self_test(long* bias_regular, long* bias_st, int debug)
{
    ...
    if (otp_value_zero == 0)
    {
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            if (debug)
                log_i("ratio=%7.4f, threshold=%7.4f\r\n", st_shift_ratio[i] / 1.f, test.max_gyro_var / 1.f);
